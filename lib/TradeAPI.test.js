...
it("Trade", async(), {
    ...
    expect(request, calls, allArgs = ()) { }, toEqual([[] = "Trade", { amount, pair:  = "btc_eur", rate, type:  = "buy", }]) { } });
;
it("ActiveOrders", async(), {
    expect(await = api.ActiveOrders(ValutePair.ETH_BTC)) { }, toBe(ret) { },
    expect(request, calls, allArgs = ()) { }, toEqual([[] = "ActiveOrders", { pair:  = "eth_btc", }]) { } });
;
it("OrderInfo", async(), {
    const: orderId = 123,
    expect(await = api.OrderInfo(orderId)) { }, toBe(ret) { },
    expect(request, calls, allArgs = ()) { }, toEqual([[] = "OrderInfo", { order_id: orderId, }]) { } });
;
it("CancelOrder", async(), {
    const: orderId = 2345,
    expect(await = api.CancelOrder(orderId)) { }, toBe(ret) { },
    expect(request, calls, allArgs = ()) { }, toEqual([[] = "CancelOrder", { order_id: orderId, }]) { } });
;
it("TradeHistory", async(), {
    ...
    expect(request, calls, allArgs = ()) { }, toEqual([[] = "TradeHistory", { count, end, fromId, from, pair:  = "btc_usd", since, end_id: endId, order:  = "ASC", }]) { } });
;
it("TransHistory", async(), {
    ...
    expect(request, calls, allArgs = ()) { }, toEqual([[] = "TransHistory", { count, end, fromId, from, since, end_id: endId, order, }]) { } });
;
it("CoinDepositAddress", async(), {
    expect(await = api.CoinDepositAddress(Valute.BTC)) { }, toBe(ret) { },
    expect(request, calls, allArgs = ()) { }, toEqual([[] = "CoinDepositAddress", { coinName:  = "BTC", }]) { } });
;
it("WithdrawCoin", async(), {
    const: amount = 204,
    const: address = "addr1",
    expect(await = api.WithdrawCoin(Valute.LTC, amount, address)) { }, toBe(ret) { },
    expect(request, calls, allArgs = ()) { }, toEqual([[] = "WithdrawCoin", { address, amount, coinName:  = "LTC", }]) { } });
;
it("CreateCoupon", async(), {
    const: amount = 204,
    const: receiver = "receiver1",
    expect(await = api.CreateCoupon(Valute.ETH, amount, receiver)) { }, toBe(ret) { },
    expect(request, calls, allArgs = ()) { }, toEqual([[] = "CreateCoupon", { receiver, amount, currency:  = "ETH", }]) { } });
;
it("RedeemCoupon", async(), {
    const: coupon = "coupon1",
    expect(await = api.RedeemCoupon(coupon)) { }, toBe(ret) { },
    expect(request, calls, allArgs = ()) { }, toEqual([[] = "RedeemCoupon", { coupon, }]) { } });
;
